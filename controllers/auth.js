    ...
    return User
      ...
      .exec((err, user) => {
        ...
        return newUser.save((err, result) => {
          if (err) return res.status(401).json({ error: 'Error saving user in database. Try later' });
...
exports.login = (req, res) => {
  ...
  User
    .findOne({ email })
    .exec((err, user) => {
      if (err || !user) return res.status(400).json({ error: 'User with that email does not exist. Please register.' });
      // authenticate
...
exports.adminMiddleware = (req, res, next) => {
  ...
  User
    ...
    .exec((err, user) => {
      if (err || !user) return res.status(400).json({ error: 'User not found' });
      if (user.role !== 'admin') return res.status(400).json({ error: 'Admin resource. Access denied' });
      req.profile = user;
      return next();
    });
};
...
exports.forgotPassword = (req, res) => {
  ...
  User
    .findOne({ email })
    .exec((err, user) => {
      if (err || !user) return res.status(400).json({ error: 'User with that email does not exist' });
      ...
      return user.updateOne({ resetPasswordLink: token }, (err, success) => {
        if (err) return res.status(400).json({ error: 'Password reset failed. Try later.' });
        const sendEmail = ses.sendEmail(params).promise(); 
      
        return sendEmail
          .then(data => {
            console.log('ses reset pw success', data);
            return res.json({ message: `Email has been sent to ${ email }. Click on the link to reset your password` });
          })
          .catch(error => {
            console.log('ses reset pw failed', error);
            return res.json({ message: `We could not vefiry your email. Try later.` });
          });
      });
    });
};
...
exports.canUpdateDeleteLink = (req, res, next) => {
  ...
  Link
    ...
    .exec((err, data) => {
      ...
      const authorizedUser = data.postedBy._id.toString() === req.user._id.toString();
